...
class StatisticsSPSentence
{
  ...
  {
    $this->_sessionKey = STATS_SETTINGS_SESSION_PREFIX . "spBySentence";
    if (!isset($_SESSION[$this->_sessionKey]))
      $_SESSION[$this->_sessionKey] = array("chapter" => 16, "types" => array("phonetic"));
    ...
    if (isset($path_pieces[2]) && $path_pieces[2] == "all")
    {
      ...
      if (sizeof($_SESSION[$this->_sessionKey]["types"]) > 0)
      {
        ...
        header("Location: " . URL_PATH . "/stats/spelling-by-sentence/");
        exit();
      }
    }
    foreach ($this->_spellingTypeXML->xpath("//type") as $spType)
    {
        if ($toggle_all)
        {
          $_SESSION[$this->_sessionKey]["types"][] = (string)$spType;
          continue;
        }
        ...
        {
          if (in_array((string)$spType, $_SESSION[$this->_sessionKey]["types"]))
            unset($_SESSION[$this->_sessionKey]["types"][array_search((string)$spType, $_SESSION[$this->_sessionKey]["types"])]);
          else if (!in_array((string)$spType, $_SESSION[$this->_sessionKey]["types"]))
            $_SESSION[$this->_sessionKey]["types"][] = (string)$spType;
          header("Location: " . URL_PATH . "/stats/spelling-by-sentence/");
          exit();
        }
    ...
    $svgStyleSheet->load(DOCUMENT_ROOT . "/documents/spelling type chart.xsl");
    ...
    $this->_xsltProcessor->setParameter('', "spellingTypes", implode(" ", $_SESSION[$this->_sessionKey]["types"]));
    $this->_xsltProcessor->importStylesheet($svgStyleSheet);
    return true;
  }
  function getHandle() { return "stats"; }
  function getTitle() { return Statistics::getTitle("Spelling Mistakes by Sentence"); }
  public static function outputSidebar()
  {
    Statistics::outputSidebar("spelling-by-sentence");
  }
  function outputBody()
  {
    echo "<h1>Spelling Mistakes By Sentence</h1>\n";
    echo "<p>This interactive chart tracks the number of spelling mistakes of each type in each sentence of the selected chapter. Use the checkboxes to select the desired types and the drop down menu to select the chapter.</p>";
    echo "<select onchange=\"window.location = '" . URL_PATH . "/stats/spelling-by-sentence/change-chapter/' + " .
	...
	(in_array((string)$spType, $_SESSION[$this->_sessionKey]["types"]) ?
	" style=\"color:black;background-color:" . $spType->attributes()->color . ";\"" : "") .
        "\"><input type=\"checkbox\" id=\"sentence-sp-toggle-" .
        (string)$spType . "\"" . (in_array((string)$spType, $_SESSION[$this->_sessionKey]["types"]) ? " checked=\"checked\"" : "") .
        " onclick=\"window.location = '" . URL_PATH . "/stats/spelling-by-sentence/" . str_replace("?", "q", (string)$spType) .
        "/'\"/><label for=\"sentence-sp-toggle-" . (string)$spType . "\">" . ucfirst((string)$spType) . "</label></div>";
    echo "<button onclick=\"window.location = '" . URL_PATH . "/stats/spelling-by-sentence/all/';\">Toggle All</button>\n";
